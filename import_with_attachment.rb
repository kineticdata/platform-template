...
Dir["#{core_path}/space/datastore/forms/**/submissions*.ndjson"].sort.each do |filename|
  ...
  attachment_form =  space_sdk.find_datastore_form(form_slug, {"include" => "fields.details"})
  ...
  File.readlines(filename).each do |line| 
    ...
    submission['values'].select{ |field, value| attachement_files.include?(field)}.each{ |field,value|
       value.each_with_index{ | value, index | 
        index_to_submissionid = value['link'].split('/').index('submissions')+1
        previous_submissionid = value['link'].split('/')[index_to_submissionid]
        attachment_path = "#{core_path}/space/datastore/forms/#{form_slug}/#{previous_submissionid}/#{field}/#{value['name']}"
        submission['values'][field][index]['link'] = attachment_path
       } 
    }
    ...
    space_sdk.add_datastore_submission_with_attachment(form_slug, body)
  end
end
=begin
# ------------------------------------------------------------------------------
